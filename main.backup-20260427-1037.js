...
function drawBg(){
  ...
  for(let i=0;i<W+STRIP;i+=36){
    const fx=((i+8-bgOffset*0.6)%STRIP+STRIP)%STRIP;
    if(fx>=W+20) continue;
    const ci=Math.floor(i/36)%wfCols.length;
    const fy=groundTop+3+((i*7)%8);
    const psz=8+(i*3)%6;   // 8–13px center — much more visible
    // Stem
    bgCtx.fillStyle='#3A9010';
    bgCtx.fillRect(fx+psz/2-1,fy+psz,3,8+((i*5)%6));
    // Center
    bgCtx.fillStyle=wfCols[ci];
    bgCtx.fillRect(fx,fy,psz,psz);
    // Four petals (each petal = (psz-1) wide)
    const pw=psz-1;
    bgCtx.fillStyle='rgba(255,255,255,0.75)';
    bgCtx.fillRect(fx-pw,fy+(psz-pw)/2,pw,pw);     // left
    bgCtx.fillRect(fx+psz,fy+(psz-pw)/2,pw,pw);    // right
    bgCtx.fillRect(fx+(psz-pw)/2,fy-pw,pw,pw);     // top
    bgCtx.fillRect(fx+(psz-pw)/2,fy+psz,pw,pw);    // bottom
    // tiny highlight dot on center
...
function onBasketClick(e){
  ...
  // Check puppy click
  const puppyX=W2*0.5, puppyY=H2*0.38+160;
  ...
  for(let i = 0; i < 3; i++){
    if(Math.abs(mx - centers[i]) < 60 && my > 50 && my < 300){
...
function drawBasketBg(ctx, W, H){
  // Sage green sky gradient
  const sky=ctx.createLinearGradient(0,0,0,H);
  sky.addColorStop(0,'#8FAF7E');sky.addColorStop(0.45,'#A8C494');sky.addColorStop(0.75,'#C4D8A8');sky.addColorStop(1,'#D0E0B8');
  ctx.fillStyle=sky;ctx.fillRect(0,0,W,H);

  // Picnic blanket
  const blY=H*0.38;
  const checkSize=32;
  for(let row=0;row<Math.ceil((H-blY)/checkSize)+1;row++){
    for(let col=0;col<Math.ceil(W/checkSize)+1;col++){
      ctx.fillStyle=(row+col)%2===0?'#F0D8D8':'#FAFAFA';
      ctx.fillRect(col*checkSize,blY+row*checkSize,checkSize,checkSize);
    }
  }
  ctx.fillStyle='#D8A898';ctx.fillRect(0,blY,W,8);
  for(let x=0;x<W;x+=18){ctx.fillStyle=(x/18)%2===0?'#C89080':'#E8B8A8';ctx.fillRect(x,blY,9,8);}
  ctx.strokeStyle='rgba(190,120,110,0.2)';ctx.lineWidth=1;
  for(let row=0;row<(H-blY)/checkSize+1;row++){ctx.beginPath();ctx.moveTo(0,blY+row*checkSize);ctx.lineTo(W,blY+row*checkSize);ctx.stroke();}
  for(let col=0;col<W/checkSize+1;col++){ctx.beginPath();ctx.moveTo(col*checkSize,blY);ctx.lineTo(col*checkSize,H);ctx.stroke();}

  // Grass sides
  ctx.fillStyle='#78CC50';ctx.fillRect(0,blY-14,38,H-blY+14);ctx.fillRect(W-38,blY-14,38,H-blY+14);
  ctx.fillStyle='#90DD68';ctx.fillRect(0,blY-14,22,5);ctx.fillRect(W-22,blY-14,22,5);

  // Sage green sky soft clouds
  const t=basketBgTime*0.01;
  ctx.fillStyle='rgba(255,255,255,0.55)';
  [[W*0.14,H*0.09,72],[W*0.52,H*0.06,58],[W*0.84,H*0.12,48]].forEach(([cx,cy,cw])=>{
    const ox=Math.sin(t+cx*0.01)*10;
    const puffs=[[0.5,0.6,0.48,0.55],[0.24,0.72,0.26,0.42],[0.76,0.72,0.26,0.42],[0.36,0.3,0.22,0.38],[0.64,0.3,0.22,0.38]];
    puffs.forEach(([fx,fy,rx,ry])=>{const h2=cw*0.42;ctx.beginPath();ctx.ellipse(cx+ox+cw*fx,cy+h2*fy,cw*rx,h2*ry,0,0,Math.PI*2);ctx.fill();});
  });

  // Napping puppy — centered on the blanket
  drawNappingDog(ctx, W*0.5, blY+160, basketBgTime);

  // Bottom hint
  ctx.fillStyle='rgba(230,245,220,0.92)';ctx.fillRect(0,H-36,W,36);
  ctx.fillStyle='rgba(40,80,50,0.9)';ctx.font='7px "Press Start 2P",monospace';ctx.textAlign='center';
...
function basketLoop(){
  ...
  centerXs.forEach((cx,i)=>{
    ...
    drawPicnicBasket(basketCtx,cx,60,currentBaskets[i],isOpen,shake);
  });
...
function submitGuestbook(){
  ...
  document.getElementById("gb-name").value="";document.getElementById("gb-email").value="";document.getElementById("gb-msg").value="";
